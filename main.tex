...
g^*(y) &= \prod_{a_i > y} (1-P(a_i)) \frac{P'(x)}{P^2(x) Q(x)}
...
dF_i(x_i) &= h_i f^*(x_i) dx_i, \qquad a_i < x_i < a_{i+1} \qquad i=1, \dots, n \\
dG_j(y_j) &= k_j g^*(y_j) dy_j, \qquad b_j < y_j < b_{j+1} \qquad j=1, \dots, m \\
1 + 2 \alpha &= D_n + 2h_n \\
1 + 2 \beta &= E_n + 2k_n \\
h_i &= (1-D_i) h_{i+1} \\
k_j &= (1-E_j) k_{j+1} \\
